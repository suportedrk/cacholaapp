-- ============================================================
-- Migration 125 — BI: conversão dos guards para check_permission
--                  em 3 módulos (Fase 3, Parte 2)
-- Criado em: 2026-05-29
-- Referência: docs/rbac/fase-a-bi.md (MAPA RPC→módulo canônico)
--             + migrations 120 (helper) e 123 (golden pattern do Vendas)
--             + Aprendizados 5 (códigos PT-BR) e 6 (dump do banco vivo)
--
-- Em cima do backfill já aplicado (migration 124, em produção): o diretor
-- já tem bi/bi_atendimento/bi_vendas.view ANTES desta troca de trava.
--
-- ── DECISÕES DO DONO APLICADAS (FASE A) ───────────────────────
--   G1: split em 3 módulos de catálogo (bi / bi_atendimento / bi_vendas).
--   G2=C: bi_vendas vira super_admin+diretor (gerente/financeiro SAEM);
--         fecha o acesso vestigial via JWT desses cargos.
--   G3: converter bi/bi_atendimento/bi_vendas remove o acesso-JWT de
--       gerente/financeiro (efeito desejado).
--   G4: drill-down de vendedora → bi_atendimento.
--   G5: can_view_festa_values() PERMANECE intocado (mascara colunas de
--       valor; ortogonal ao guard de módulo). NÃO é tocado nesta migration.
--
-- ── O QUE ESTA MIGRATION FAZ ──────────────────────────────────
--   1a. Limpa overrides dormentes em user_permissions: remove qualquer
--       grant nos 3 módulos BI de usuários com role FORA de
--       (super_admin, diretor). Pela auditoria 2026-05-29 (prod), atinge
--       apenas o gerente brunocasaletti@gmail.com (15 grants: 5 ações ×
--       3 módulos) — conta de teste do dono, limpeza aprovada. Sem essa
--       limpeza, esses grants "acordariam" na conversão (gerente ganharia
--       /bi via bi.view e manteria bi_vendas, contra G2=C/G3).
--   1b. Template (role_permissions): remove gerente e financeiro de
--       bi_vendas (G2=C). bi e bi_atendimento NÃO são tocados.
--   1c. Converte os 17 RPCs get_bi_* trocando SOMENTE o bloco de guarda
--       (IF NOT EXISTS ... role IN ... RAISE) por
--       PERFORM public.check_permission_or_raise('<modulo>','view'),
--       conforme o MAPA da FASE A. Corpo extraído via pg_get_functiondef
--       do banco VIVO (produção) — preserva hotfixes e can_view_festa_values().
--         • Visão Geral (5)  → 'bi'
--         • Drill-down  (3)  → 'bi_atendimento'
--         • Vendas Real.(9)  → 'bi_vendas'
--
-- super_admin é bypass dentro de check_permission() (early-return).
--
-- Transação única. Idempotente (CREATE OR REPLACE; DELETE ON state;
-- DO-block de pós-condição autovalidante). NOTIFY pgrst ao final.
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- Pré-condição: catálogo tem 'view' nos 3 módulos (códigos exatos)
-- ------------------------------------------------------------
DO $mig$
DECLARE
  v_count INT;
BEGIN
  ...
  WHERE module_code IN ('bi','bi_atendimento','bi_vendas')
    AND code = 'view' AND kind = 'action';
  IF v_count <> 3 THEN
    RAISE EXCEPTION 'Abortando: esperava 3 ações view (bi/bi_atendimento/bi_vendas), encontrei %.', v_count;
  END IF;
...
-- ------------------------------------------------------------
-- 1a — Limpeza de overrides dormentes em user_permissions
--      (roles fora de super_admin/diretor nos 3 módulos BI)
-- ------------------------------------------------------------
DO $mig$
DECLARE
  v_before INT;
...
BEGIN
  ...
  RAISE NOTICE '1a — grants dormentes (roles fora de sa/diretor) antes: %', v_before;
  ...
  WHERE up.module IN ('bi','bi_atendimento','bi_vendas')
    AND u.role NOT IN ('super_admin','diretor');
  RAISE NOTICE '1a — grants dormentes removidos: %, restantes: %', v_before - v_after, v_after;

  IF v_after <> 0 THEN
    RAISE EXCEPTION '1a falhou: ainda restam % grants dormentes nos módulos BI.', v_after;
  END IF;
END
$mig$;

-- ------------------------------------------------------------
-- 1b — Template: remove gerente e financeiro de bi_vendas (G2=C)
-- ------------------------------------------------------------
DO $mig$
DECLARE
  v_roles TEXT;
BEGIN
  ...
  WHERE module_code = 'bi_vendas'
    AND role_code IN ('gerente','financeiro');

  SELECT string_agg(DISTINCT role_code, ',' ORDER BY role_code) INTO v_roles
  FROM public.role_permissions
  WHERE module_code = 'bi_vendas';
  RAISE NOTICE '1b — bi_vendas template roles agora: %', v_roles;

  IF EXISTS (
    SELECT 1 FROM public.role_permissions
    WHERE module_code = 'bi_vendas' AND role_code IN ('gerente','financeiro')
  ) THEN
    RAISE EXCEPTION '1b falhou: gerente/financeiro ainda no template de bi_vendas.';
  END IF;
END
$mig$;

-- ============================================================
-- 1c — Conversão dos 17 RPCs (corpo dump do banco vivo de produção;
--      apenas o bloco de guarda foi trocado)
-- ============================================================

-- ============================================================
-- VISAO GERAL -> bi.view (5 RPCs)
-- ============================================================

-- ── get_bi_conversion_data → bi.view ──
...
-- ── get_bi_lead_origin_breakdown → bi.view ──
...
-- ── get_bi_unit_comparison → bi.view ──
...
-- ============================================================
-- DRILL-DOWN -> bi_atendimento.view (3 RPCs)
-- ============================================================

-- ── get_bi_sellers_ranking → bi_atendimento.view ──
...
-- ── get_bi_seller_history → bi_atendimento.view ──
...
-- ── get_bi_seller_funnel → bi_atendimento.view ──
...
-- ============================================================
-- VENDAS REALIZADAS -> bi_vendas.view (9 RPCs)
-- ============================================================

-- ── get_bi_sales_kpi → bi_vendas.view ──
...
-- ── get_bi_sales_ranking → bi_vendas.view ──
...
-- ── get_bi_seller_orders → bi_vendas.view ──
...
-- ── get_bi_category_kpi → bi_vendas.view ──
...
-- ── get_bi_category_ranking → bi_vendas.view ──
...
-- ── get_bi_category_drilldown → bi_vendas.view ──
...
-- ── get_bi_category_cross_unit → bi_vendas.view ──
...
-- ── get_bi_adoption_kpi → bi_vendas.view ──
...
-- ── get_bi_adoption_ranking → bi_vendas.view ──
...
-- ============================================================
-- Pós-condição estrutural: cada um dos 17 RPCs deve conter
-- check_permission_or_raise com o módulo CORRETO e ZERO 'role IN'.
-- ============================================================
DO $mig$
DECLARE
  r            RECORD;
  ...
  expected     CONSTANT TEXT[][] := ARRAY[
    ['get_bi_conversion_data','bi'],
    ['get_bi_funnel_data','bi'],
    ['get_bi_lead_origin_breakdown','bi'],
    ['get_bi_sales_metrics','bi'],
    ['get_bi_unit_comparison','bi'],
    ['get_bi_sellers_ranking','bi_atendimento'],
    ['get_bi_seller_history','bi_atendimento'],
    ['get_bi_seller_funnel','bi_atendimento'],
    ['get_bi_sales_kpi','bi_vendas'],
    ['get_bi_sales_ranking','bi_vendas'],
    ['get_bi_seller_orders','bi_vendas'],
    ['get_bi_category_kpi','bi_vendas'],
    ['get_bi_category_ranking','bi_vendas'],
    ['get_bi_category_drilldown','bi_vendas'],
    ['get_bi_category_cross_unit','bi_vendas'],
    ['get_bi_adoption_kpi','bi_vendas'],
    ['get_bi_adoption_ranking','bi_vendas']
  ];
  i INT;
BEGIN
  FOR i IN 1 .. array_length(expected, 1) LOOP
    ...
    WHERE n.nspname = 'public' AND p.proname = expected[i][1];

    v_expected := format('check_permission_or_raise(''%s'', ''view'')', expected[i][2]);

    IF v_def IS NULL THEN
      RAISE WARNING 'pós-cond: função % não encontrada', expected[i][1];
      v_fail := v_fail + 1;
    ELSIF position(v_expected IN v_def) = 0 THEN
      RAISE WARNING 'pós-cond: % NÃO contém %', expected[i][1], v_expected;
      v_fail := v_fail + 1;
    ELSIF v_def ~ 'role IN' THEN
      RAISE WARNING 'pós-cond: % ainda contém "role IN" residual', expected[i][1];
      v_fail := v_fail + 1;
    END IF;
  END LOOP;

  IF v_fail > 0 THEN
    RAISE EXCEPTION 'Pós-condição estrutural falhou em % de 17 RPCs.', v_fail;
  END IF;
  RAISE NOTICE 'OK: 17/17 RPCs convertidos com módulo correto e zero role IN.';
END
$mig$;
