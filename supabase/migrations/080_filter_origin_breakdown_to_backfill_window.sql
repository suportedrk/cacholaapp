...
AS $$
  ...
  WHERE d.unit_id = p_unit_id
    AND d.ploomes_create_date >= '2025-05-01'::DATE
    AND d.ploomes_create_date >= NOW() - (p_period_months || ' months')::INTERVAL
  GROUP BY 1, 2
  ORDER BY 1 ASC, 2 ASC;
$$;

COMMENT ON FUNCTION public.get_bi_lead_origin_breakdown IS
  'Agrega leads (deals do Ploomes) por mês de criação e categoria de origem (8 categorias derivadas das 13 origens cruas). Usado pelo painel BI Origem dos Leads. Filtra deals anteriores a 2025-05-01 (janela do backfill — deals mais antigos têm origin_id NULL e distorceriam o gráfico). Mapeamento 13->8 vive aqui (única fonte da verdade).';
