-- ============================================================
-- Migration 170 — Guard interno (defense-in-depth) nas RPCs de estoque da Decoração
-- ============================================================
-- Contexto (Segurança/LGPD — hardening, OWASP A01):
--   As RPCs de movimentação de estoque da Decoração são SECURITY INVOKER e hoje
--   confiam 100% na RLS das tabelas-alvo (decoracao_estoque_saldo já exige
--   check_permission(...,'decoracao',<action>) em todo CRUD — verificado). Como
--   defense-in-depth, adicionamos uma checagem explícita no TOPO de cada RPC:
--       PERFORM public.check_permission_or_raise('decoracao', 'edit');
--   Assim, se a RLS de alguma tabela for afrouxada no futuro, a RPC ainda barra
--   quem não tem 'decoracao:edit' (super_admin é bypassado dentro de
--   check_permission). Helper já existe (migration 120).
--
--   Ação UNIFORME 'edit' nas 5 RPCs (gate de escrita), de propósito: embora
--   criar_transferencia faça INSERT (RLS 'create' na tabela de transferências),
--   toda RPC também movimenta o saldo (RLS 'edit'). Verificado no template
--   role_permissions: NENHUM cargo tem 'decoracao:create' sem 'decoracao:edit'
--   (vêm sempre juntos), então o guard 'edit' não bloqueia operador legítimo. A
--   RLS de cada tabela continua aplicando a ação precisa (create/edit/delete).
--
-- As funções abaixo são as definições CANÔNICAS atuais (pg_get_functiondef),
-- recriadas via CREATE OR REPLACE com APENAS a linha de guard inserida logo após
-- o BEGIN — nenhuma outra alteração de corpo. Permanecem SECURITY INVOKER e
-- SET search_path = public. CREATE OR REPLACE preserva owner e grants.
--
-- Idempotente. Sem BEGIN/COMMIT soltos nem NOTIFY pgrst (a esteira cuida).
-- ============================================================
