-- ══════════════════════════════════════════════════════════════
-- Migration 142 — Foto por variação de item de decoração
-- Adiciona foto_path TEXT NULL em decoracao_item_variacoes e
-- atualiza as duas RPCs atômicas para persistir o campo.
-- ══════════════════════════════════════════════════════════════
...
-- ── 2. Recriar create_decoracao_item_with_variacoes ───────────
-- Corpo verbatim da migration 104 + foto_path no INSERT de variações.
 ...
 BEGIN
   ...
   LOOP
     INSERT INTO public.decoracao_item_variacoes (
       item_id, tamanho, cor, detalhe, ordem, foto_path
     )
     VALUES (
       v_item_id,
...
-- ── 3. Recriar update_decoracao_item_with_variacoes ───────────
-- Corpo verbatim da migration 104 + foto_path no INSERT e UPDATE de variações.
   ...
   LOOP
     ...
     ELSE
       ...
       SET tamanho   = NULLIF(trim(v_var->>'tamanho'), ''),
           ...
           foto_path = NULLIF(v_var->>'foto_path', '')
