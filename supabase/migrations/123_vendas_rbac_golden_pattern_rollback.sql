-- ============================================================
-- Rollback da Migration 123 — Vendas: restaura guards por cargo
--
-- Reverte:
--   1. Re-cria os 10 RPCs com o guard ORIGINAL por cargo (role IN /
--      role NOT IN inline), extraído via pg_get_functiondef ANTES da
--      conversão (estado de produção em 2026-05-29). Corpos preservados
--      byte-a-byte.
--   2. Re-insere os grants de 'vendas' do gerente removidos na 123.
--
-- ⚠ LIMITAÇÃO do passo 2: a 123 deletou overrides por ROLE (qualquer
-- cargo fora de super_admin/diretor/vendedora/pos_vendas), mas não
-- registrou o conjunto exato removido. Pela auditoria de 2026-05-29 o
-- único afetado foi o gerente brunocasaletti@gmail.com com as 5 ações
-- (view/create/edit/delete/export). Este rollback re-insere exatamente
-- essas 5 para essa conta (best-effort, idempotente). Se em produção
-- houver outros overrides fora do guard no momento do rollback, eles
-- NÃO serão reconstruídos — snapshot do banco antes é recomendado.
-- NÃO toca a conta admin bruno.casaletti@grupodrk.com.br (super_admin).
--
-- Idempotente.
-- ============================================================
...
-- 1. Re-inserir grants do gerente removidos pela 123 (best-effort)
INSERT INTO public.user_permissions (user_id, unit_id, module, action, granted)
SELECT u.id, NULL::UUID, 'vendas', a, true
FROM public.users u
CROSS JOIN (VALUES ('view'),('create'),('edit'),('delete'),('export')) AS actions(a)
WHERE u.email = 'brunocasaletti@gmail.com'
  AND u.role  = 'gerente'
ON CONFLICT (user_id, unit_id, module, action) DO NOTHING;

-- 2. Restaurar os 10 RPCs com guard por cargo (definições originais)

-- ---------- get_vendas_my_kpis (restaura guard por cargo) ----------
...
-- ---------- get_vendas_daily_revenue (restaura guard por cargo) ----------
...
-- ---------- get_vendas_ranking (restaura guard por cargo) ----------
...
-- ---------- get_upsell_opportunities (restaura guard por cargo) ----------
...
-- ---------- get_upsell_count_for_user (restaura guard por cargo) ----------
...
-- ---------- get_upsell_popular_addons (restaura guard por cargo) ----------
...
-- ---------- get_recompra_aniversario_proximo (restaura guard por cargo) ----------
...
-- ---------- get_recompra_festa_passada (restaura guard por cargo) ----------
...
-- ---------- get_recompra_count_for_user (restaura guard por cargo) ----------
...
-- ---------- get_event_sales_summary (restaura guard por cargo) ----------
