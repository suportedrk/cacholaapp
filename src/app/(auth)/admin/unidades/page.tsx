...
import { Building2, Plus, Loader2, MapPin, Phone, CheckCircle2, XCircle, Settings2 } from 'lucide-react'
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {canSetup && (
            <Button
              variant="outline"
              onClick={() => router.push(ROUTES.unitSetup)}
            >
              <Settings2 className="w-4 h-4 mr-2" />
              Setup de Unidade
            </Button>
          )}
          <Button onClick={() => router.push(`${ROUTES.units}/nova`)}>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((unit) => (
            <UnitCard
              ...
              onSetup={() => router.push(`${ROUTES.units}/${unit.id}/setup`)}
            />
          ))}
        </div>
      )}
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <button
        onClick={onClick}
        className={cn(
          'w-full text-left p-4 card-interactive rounded-none',
