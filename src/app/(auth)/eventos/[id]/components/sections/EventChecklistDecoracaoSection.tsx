...
import {
  ...
  openChecklistDecoracaoPrint,
  NAO_PREENCHIDO,
  RESTRITO,
} from './checklist-decoracao-print'
import type { Event } from '@/types/database.types'

interface EventChecklistDecoracaoSectionProps {
  event: Event
  unitName: string | null
  canSeeValues: boolean
}

export function EventChecklistDecoracaoSection({ event, unitName, canSeeValues }: EventChecklistDecoracaoSectionProps) {
  const items = buildChecklistDecoracaoItems(event, canSeeValues)

  return (
    <AccordionSection title="Checklist de Decoração" icon={Sparkles}>
      <div className="space-y-3">
        <div className="flex justify-end">
          <Button
            ...
            size="sm"
            className="gap-1.5"
            onClick={() => openChecklistDecoracaoPrint({ event, unitName, canSeeValues })}
          >
