...
**GOTCHA migration 073 + 076 + 077 — mismatch RLS × user_permissions (resolvido em v1.5.11):** migration 073 renomeou `user_permissions.module` de EN para PT-BR (`events→eventos`, `providers→prestadores`, etc.), mas 64 policies RLS continuavam chamando `check_permission()` com os códigos EN antigos. Migration **076** (28/abr) adicionou CASE EN→PT-BR dentro de `check_permission()` como fix cirúrgico temporário. Migration **077** (28/abr, v1.5.11) recriou as **49 policies que usavam códigos EN** para chamarem `check_permission()` com códigos PT-BR diretamente. As 15 policies de `'checklists'` não foram tocadas (código já era PT-BR). **O CASE em `check_permission()` foi MANTIDO permanentemente** como camada defensiva de custo zero — previne regressão caso uma futura policy esqueça de usar PT-BR. Total: 64 policies auditadas, 49 atualizadas em 28/abr.
