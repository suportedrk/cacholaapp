...
**Conflito de horário:** Função Postgres `get_event_conflicts(unit_id)` retorna pares de eventos `confirmed`/`in_progress` com intervalo < 2h na mesma data/unidade. `get_conflicting_event_ids(unit_id)` retorna IDs únicos. Hooks em `src/hooks/use-event-conflicts.ts`: `useEventConflicts()`, `useConflictingEventIds()`, `useOverlappingEventIds()` (gap_minutes <= 0), `useShortGapEventIds()` (0 < gap_minutes < 120). Desabilitado quando `activeUnitId=null` (função SQL exige UUID específico).

**Conflito de horário (filtros):** Dois chips exclusivos na barra de filtros da listagem de eventos: "Conflito de horário" (overlap, vermelho, gap_minutes <= 0) e "Intervalo < 2h" (short_gap, âmbar, 0 < gap_minutes < 120). O filtro ativo é um único estado `ActiveFilter = EventStatus | 'overlap' | 'short_gap' | null` — mutuamente exclusivo com os filtros de status. Quando filtro de conflito ativo: filtragem é client-side sobre `allEvents`; `noDateFilter=true` desativa o filtro `date >= hoje` no `useEventsInfinite`; counter mostra "N eventos com conflito". Badge no `EventCard` usa `conflictType?: 'overlap' | 'short_gap' | null` (substituiu `hasConflict?: boolean`) com visual vermelho (sobreposição) ou âmbar (intervalo). Banner separado por tipo no topo da listagem.
