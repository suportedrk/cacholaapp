...
**Conflito de horário:** Função Postgres `get_event_conflicts(unit_id)` retorna pares de eventos `confirmed`/`in_progress` com intervalo < 2h na mesma data/unidade. `get_conflicting_event_ids(unit_id)` retorna IDs únicos. Hook: `useEventConflicts()` e `useConflictingEventIds()` em `src/hooks/use-event-conflicts.ts`. Banner âmbar no topo da listagem + badge inline "Conflito" no `EventCard` (prop `hasConflict?: boolean`, Set calculado uma vez na page com `useMemo`). Desabilitado quando `activeUnitId=null` (função SQL exige UUID específico).
