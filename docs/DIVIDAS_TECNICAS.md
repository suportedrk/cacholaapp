...
- [ ] Ferramentas de apoio ao executor/subagentes — 3 recomendadas em 20/06/2026, nenhuma feita ainda: (1) Conexao ao GitHub para o Claude ver o status do CI sozinho — JA REGISTRADA acima (gh CLI + token repo:read/actions:read); a mais util das 3. (2) Conexao ao banco no lado do executor (Postgres/Supabase MCP) para os agentes auditarem RLS/permissoes contra os dados reais (hoje dependem de workaround via psql). (3) Observabilidade: adotar um monitor de erros de producao (ex.: Sentry + MCP) — hoje e ZERO; um erro que atinja um cliente nao gera nenhum alerta. STATUS: PENDENTE.
