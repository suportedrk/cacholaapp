...
- **seguranca-web-appsec** (`.claude/skills/seguranca-web-appsec/`) — AppSec para Next.js + Supabase: OWASP Top 10:2025, SQLi, controle de acesso/IDOR, autenticação/sessão, XSS/CSRF, configuração segura, checklists por feature (em `references/`). Base do subagente `appsec-security-reviewer`. Use ao implementar ou revisar qualquer ponto de superfície de ataque.
